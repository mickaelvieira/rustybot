...
impl ParsedUrl {
    ...
    pub fn new(url: &str) -> ParsedUrl {
        ParsedUrl {
            ...
            parsed: Url::parse(ParsedUrl::clean_chars(url).as_str()).unwrap(),
        }
    }

    pub fn parse<S>(url: S) -> Result<ParsedUrl, &'static str>
        where S: Into<String>
    {
        let url = url.into();
        let parsed = Url::parse(ParsedUrl::clean_chars(url.as_str()).as_str());
        if parsed.is_ok() {
            return Ok(ParsedUrl {
                original: url,
